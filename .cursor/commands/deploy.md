...
ssh statex "cd payment-microservice && git pull"

pull github repos using ssh statex "cd payment-microservice && git pull && cd ../nginx-microservice && git pull && docker exec nginx-microservice nginx -t && docker exec nginx-microservice nginx -s reload"
...
Use the same nginx and database setup to manage payment-microservice:
Run: ssh statex "cd nginx-microservice && ./scripts/blue-green/deploy.sh payment-microservice"
...
Success is when payment-microservice is accessible without console or log errors.
